package ru.almaunion.statraiff;

public class AppProperties {

    /**
     * In a real app, either hide this on your server
     * or compose it of multiple concat Strings so it's not so easy to find
     */
    //public static final String BASE_64_KEY = "";
	public static final String BASE_64_KEY = "+7GqKS30InJW6WiHhcjyJVNIx5Lmv8CGGNwJA/GSo49SrNZJdZdn5tqOQ7Dq7u3BFXNrToB+9Mb/FmIPcJWDAwcIQw53rpVQcguHPS7HVDIZT++YOano9UIvU0WQB3x7UrbQnP157KND74WfhL/7B+dfdLsMLVpEcE";
    
    public static final int DONATION_AMOUNT_01 = 1; // $1
    public static final int DONATION_AMOUNT_02 = 2; // $3
    public static final int DONATION_AMOUNT_03 = 3; // $5
    
}
